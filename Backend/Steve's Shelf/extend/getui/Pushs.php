...
/* define('APPKEY', 'd72aQeMAK68toQuPeR3zw8');
define('APPID', 'Qxz7rtTjUO7oqdQBgVCyq8');
define('MASTERSECRET', 'Yymfo5gsb167vX3kaauWq2');
define('HOST', 'http://sdk.open.api.igexin.com/apiex.htm');
define('DEVICETOKEN','8d1abc6d282f6ab85caa28e90a5dca9aa9eb74b43137535e156c431fb6a55a90');  */
// 个推 修改
define('APPKEY', 'EB758j2tA56QiL9FU7RYt');
define('APPID', '6nT1wBF3yV6LGzV9BlbIt7');
define('MASTERSECRET', 'c71hBrT7Nf8ES8YP09ukv9');
define('HOST', 'http://sdk.open.api.igexin.com/apiex.htm');
define('DEVICETOKEN','8d1abc6d282f6ab85caa28e90a5dca9aa9eb74b43137535e156c431fb6a55a90');
	 
 
	
// define('CID','c54421c1acde3a5ac0c3a3d6b4610e26');
define('DT','');
define('CID1','');
define('DT1','');
define('groupName','');
...
class Pushs {
	...
	//
	//单推接口案例
	function pushMessageToSingle($msg,$clientId){
		$igt = new \IGeTui(HOST,APPKEY,MASTERSECRET);
	...
	function pushMessageToApp($msg){
		$igt = new \IGeTui(HOST,APPKEY,MASTERSECRET);
		$template = $this->IGtLinkTemplateDemo($msg);
		//个推信息体
		//基于应用消息体
	...
	function IGtLinkTemplateDemo($msg){
		$template =  new \IGtLinkTemplate();
		$template ->set_appId(APPID);//应用appid
		$template ->set_appkey(APPKEY);//应用appkey
		$template ->set_title($msg['title']);//通知栏标题
		$template ->set_text($msg['content']);//通知栏内容
		$template ->set_logo("");//通知栏logo
		$template ->set_isRing(true);//是否响铃
		$template ->set_isVibrate(true);//是否震动
		$template ->set_isClearable(true);//通知栏是否可清除
		$template ->set_url($msg['curl']);//打开连接地址
		//$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
		return $template;
	}
	...
	function IGtNotificationTemplateDemo($msg){
		...
		$template->set_title($msg['title']);//通知栏标题
		$template->set_text($msg['text']);//通知栏内容
	...
	function getTypes($msg,$data){
		...
		*/
		...
		if(count($iosArray)>0){
			...
			$igt = new \IGeTui(HOST,APPKEY,MASTERSECRET);
			$contentId = $igt->getAPNContentId(APPID,$msgCache[1]);
            $igt->pushAPNMessageToList(APPID, $contentId, $iosArray);
		}
		//dump($andoreArray);
		if(count($andoreArray)>0){
			if(count($andoreArray)>=2){
				//群推
				$res = $this->pushMessageToList($msg,$andoreArray);
			}else{
				//单推
				$res = $this->pushMessageToSingle($msg,$andoreArray[0]);
			}
			return($res);
		}
		return;
