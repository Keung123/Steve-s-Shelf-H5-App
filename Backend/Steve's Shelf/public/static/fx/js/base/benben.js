...
//				    gP.navigator.setStatusBarBackground('#22512F');
...
//				alert(json.url+'?'+json2url(json.data));
			...
			case 'post':
				...
				console.log(json2url(json.data))
			        ...
			        if(arrayData[index].title == '照相机'){
				        	dongyi.camera({
				        		succFn:function(path,name){
				        			...
				        			var path1 = path
				        			var geshi = '.' + path.substr(path.lastIndexOf('.')+1)
									var path = path.substr(0,path.lastIndexOf('.')+1) + geshi
				        			//压缩图片
									plus.zip.compressImage({
											src:path1,
											dst:path,
											quality:20,   //图片质量 越小越差
											overwrite:true //允许覆盖
										},
										function() {
											mui.toast('上传成功')
							        		if(jsonData && jsonData.isUpload){
							        			dongyi.uploadFiles(jsonData.uploadUrl,path,jsonData.userId,jsonData.type,jsonData.n,jsonData.iszip,function(imgPath){
	...
	showWaiting:function(watingPrompt){
		mui.plusReady(function(){
			if(watingPrompt){
				...
				gP.nativeUI.showWaiting('',{color:"#fff",width:"120px",height:"120px",size:0,background:"rgba(255,255,255,0.7)",loading:{icon:"../img/waiting.png",height:"80px",interval:200}});	
			}
...
//	              	 succFn(img_url.img.replace('txt','jpg'));
	...
	getAppVersion:function(callback){
		gM.plusReady(function(){
			callback && callback(gP.os.version);
		});
	return gP.os.version;
	},
	/**
	 * 滑动九宫格
	 */
	...
	pay:function(json){
		gM.ready(function(){		
			...
			if(json.type=='alipay' || json.type=='wxpay'){
				if(json.type=='wxpay'){
				url = json.url+"?type="+json.type+"&log_id="+json.log_id;
				}else{
				url = json.url+"?type="+json.type+"&log_id="+json.log_id;
				}
			}else{
	...
	share:function(id,ex,jsonData){
		...
		gM.ready(function(){
			...
			/**
			   ...
			   * @param {String} id
			   */
			function shareAction( id,ex,shares ) {
				var s=null;
				if(!id||!(s=shares[id])){
...
//		var arr = [appid,'usercenter'];
//		ca.sendNotice(arr,'my_group');
