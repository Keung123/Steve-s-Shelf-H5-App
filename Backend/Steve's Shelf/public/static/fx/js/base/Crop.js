...
//      for(key in offsmet.frame) {
//          offset.frae[key] = Math.floor(offset.frame[key]);
//      }
    ...
    veCropProtytype.generate = function(fn) {
        ...
        img.onload = function() {
            ...
            canvas.width = frameOffset.width*2.5;//裁剪框的宽高
            canvas.height = frameOffset.height*2.5;
    ...
    veCropProtytype.filterImage = function(image, coor) {
        ...
        return canvas.toDataURL('image/webp', 1);//压缩质量为.5
    };
    ...
    veCropProtytype.initTouch = function() {
        ...
        var _h,_w,confineWT,confineW,confineHT,confineH;
        ...
        Touch.on(frame, 'touchstart', function (ev) {
            ev.preventDefault();
              ...
              _h =  _img.height*currScale,
              _w =  _img.width*currScale;         //计算图片实际大小
               confineWT = (_w -_img.width)/2;     //图片是向两边扩大的，所以每边是一半
               confineW = _w -document.body.clientWidth-confineWT;
         ////
              confineHT =  ( _h - _img.height)/2;
              confineH =   _h - document.body.clientWidth- confineHT;
...
//           console.log(_w,_h)
        //    //控制X轴
              if( currOffx<0 && currOffx < (-confineW)){
                currOffx = -confineW;
              }else if(currOffx > confineWT ){
                currOffx = confineWT; 
              } 
        //  ////控制Y轴
              if(currOffy > confineHT){
                currOffy = confineHT;
              }else if(currOffy<0 && currOffy < (-confineH)){
                currOffy = -confineH;
              }
        ...
        Touch.on(frame, 'pinch', function(ev) {
            if(typeof ev.scale != 'undefined') {
                ...
                var _marke = true;
                var relWidth =  $('#frameImg').width() *currScale; 
                if( relWidth < 200){
                    _marke = false;
//                  alert(1);
                    currScale = 1;
                ...
                if(_marke){_this.formatTransform(0, 0, currScale, _this.param.deg)};
            }
        });
        ...
        Touch.on(frame, 'pinchend', function() { 
            ...
            setTimeout(function(){_this.formatTransform(0, 0, currScale, _this.param.deg)},200);
        });
    };
