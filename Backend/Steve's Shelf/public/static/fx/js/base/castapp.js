		...
		gM.plusReady(function(){
			plus.nativeUI.actionSheet( {
			    ...
			    function(e){    	
			        ...
			        console.log(arrayData[index].title,"index");
			       
			        if(arrayData[index].title == '照相机'){
			        	//console.log(2222222222222222222);
				        	dongyi.camera({
				        		succFn:function(path,name){
				        			console.log(path,name,"上传图片");
...
//			gP.nativeUI.closeWaiting();	
	 ...
	 * @param {Object} phone
	 */
	callPhone:function(targetPhone,confirm){
		var status;
		if(confirm){
			status = confirm;
		}else{
		...
		gP.device.dial(targetPhone,status);
	},
	/**	
	 * 邮件
	        ...
	        function(t,status){ 
	            ...
	            if(status==200){
	               ...
	               // console.log(JSON.stringify(responseText));
	                ...
	                var files = json.data.uploadkey;
	             //  console.log(JSON.stringify(files));
	                var img_url = json.data.img;   
	               // console.log(img_url);
	                succFn(img_url);
	                dongyi.closeWaiting();
	            }else{
	...
	dblclickExit:function(){
		var cur = ca.getCurrentInterface();
		var first = null;
		gM.back = function() {
			if (!first) {
	...
	pay:function(json){
		gM.ready(function(){		
			...
			var out_trade_no_cz =json.out_trade_no_cz;
			var out_trade_no_gm =json.out_trade_no_gm;
			var url = '';
			console.log(json.type);
			if(json.type=='alipay' || json.type=='wxpay'){
				if(json.type=='wxpay'){
				url = json.url+"?type="+json.type+"&payid=";
				}else{
				url = json.url+"?type="+json.type+"&out_trade_no_cz="+json.out_trade_no_cz+"&out_trade_no_gm="+json.out_trade_no_gm+"&payid=";
				}
			}else{
				dongyi.prompt("不支持此支付通道！");
				return;
			}
			url+='&appid=benben&total=';
