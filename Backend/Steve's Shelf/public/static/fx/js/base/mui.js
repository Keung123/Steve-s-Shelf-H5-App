				...
				if (indicators.length > 0) { //图片轮播
					...
					if (number) { //图文表格
						...
						for(var i in controlItems){
							//console.log(controlItems[i].className);
							if(controlItems[i].className.indexOf('mui-active')!=-1){
								console.log(controlItems[i].type);
								localStorage.setItem('type',controlItems[i].type);
								huoqu();
							}
						}
