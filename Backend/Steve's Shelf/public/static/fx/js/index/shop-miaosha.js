...
//					}
					...
					str += '<p class="text_overflow" style="display:'+menu_xs+'"><em>'+res.data.active_name+'</em>' + res.data.abstract_content + '</p>' +
						...
						'<div class="price">' +
						'<span><em>¥</em>' + res.data.limit_price + '</span>' +
						'<i class="vip" style="display:'+show_zhanshi+'">'+dz_vip+'¥' + vip_dz_price + '</i>' +
						'<del class="vip" style="display:'+show_price_zhanshi+'">¥' + res.data.price + '</del>' +	
...
//	}, 1500);
				...
				if(res.status == 1) {
					for(var i in data) {
						...
						for(var j in data[i].mate_thumb) {
							str += '<img src="' + hetao.url2 + '' + data[i].mate_thumb[j] + '" data-preview-src="" data-preview-group="1" />'
						}
						str += '</div>' +
							'<div class="collect">' +
	...
	$('.buy').click(function() {
        ...
        if(!user_id) {
            ...
            ca.newInterface({
                url: '../car/write_order.html?goods_id=' + goods_id + '&' + sku_id + '&' + 'huodong' +'&'+5,
