	...
	var active_type_id = window.location.search.split('=')[1].split('&')[1];
	var bargain_id = window.location.search.split('=')[1].split('&')[2];
	...
	$('.buy').click(function() {
        ...
        ca.confirm({
            ...
            callback:function(data){
                if(data){
                    ca.get({
                        url:hetao.url+'Activity/openBargain',
                        data:obj,
                        succFn:function(data){
                            var res=JSON.parse(data);
                            console.log(res)
                            if(res.status==1){
                                ca.prompt(res.msg);
                                ca.newInterface({
                                    url:'zhuli_kanjia.html?id='+bargain_id,
                                    id:'zhuli_kanjia.html'
                                })
                            }else{
                                ca.prompt(res.msg);
                            }
