            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        {field: 'total', title: "返现总额"},
                        /*{field: 'operate', title: __('Operate'), events: Table.api.events.operate, formatter:function(value,row){
                                var html = [];
                                html.push('<a href="'+lists_url+'?user_id='+row.user_id+'" title="销售订单列表" class="btn btn-success btn-addtabs btn-xs">订单列表</a>');
                                return html.join(' ');
                            }}*/
                        {field: 'operate', title: __('Operate'),events: Controller.api.events.operate, formatter: function(value, row, index){
                                var detail="";
                                detail+='<a class="btn btn-xs btn-success saleslist" title="销售订单列表">订单列表</a> ';
                                return detail;
                            }}
                    ]
                ]
            ...
            events: {//绑定事件的方法
                operate: $.extend({
                    'click .saleslist': function (e, value, row, index) {
                        e.stopPropagation();
                        Backend.api.open("/admin/sales/saleslist?user_id="+ row.user_id, '销售订单列表');
                    },
