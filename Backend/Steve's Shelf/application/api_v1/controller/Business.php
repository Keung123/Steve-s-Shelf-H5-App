...
class Business extends Common{
    ...
    */
    public function uploadLicense(){
        ...
        $BusinessModel=new BusinessService();
        $b_data = $BusinessModel->getLicenseData($uid);
		$data = [
			'b_uid'=> $uid, //用户ID'
		...
		if($b_data){
			...
			$res = $BusinessModel->updateLicense($data,$b_data['b_id']);
		}else{ 
			$res = $BusinessModel->addLicense($data);
		} 
    ...
    */
    public function judgeLicense(){
        $uid = $this->uid;
		/*$BusinessModel=new BusinessService();
        $res = $BusinessModel->judgeLicense($uid);*/
		$res = Db::name('business')->where('b_uid',$uid)->find();
		$data = ['bus_status'=>0];
		...
		/*if(!$res){
			return $this->json('', 0, '未上传可以提交');
		}else if($res['status'] == 2){
			return $this->json($res, -1, '未通过');
		}else if($res['status'] == 1){
			return $this->json($res, 2, '已通过');
		}*/
    ...
    */
    public function uploadInvoice(){
        ...
        $BusinessModel=new BusinessService();
        $res = $BusinessModel->uploadInvoice($data);
        if($res){
