...
class Common{
	...
	*/
    ...
    public function imgUpload(){
    	...
    	$new_w = 150;
    	$new_h = 150;
    	$new_obj = imagecreatetruecolor($new_w, $new_h);
    	imagecopyresampled($new_obj, $obj, 0, 0, 0, 0, $new_w, $new_h, $width, $height);
    	...
    	// if(!move_uploaded_file($oImg['tmp_name'], $tmp_p_dir.$tmp_c_dir.DS.$tmp_name)){
    	// 	return $this->json('', 0, '移动图片失败');
    	// }
    	// else{
		 
    		return $this->json(['img' => request()->domain().'/uploads/'.$tmp_c_dir.'/'.$tmp_name]);
    	// }
    }
        ...
        if($imgbase){
            ...
            foreach($all_img as $val){
                if($val){
                    ...
                    //生成目录
                    $img_p_dir = ROOT_PATH.'public'.DS.'uploads'.DS;
