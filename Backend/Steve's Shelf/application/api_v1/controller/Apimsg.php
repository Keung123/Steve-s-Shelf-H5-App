...
//        }
        ...
        foreach ($list as $k => $v) {
            ...
            $list[$k]['u'] = db("users")->where("user_id=" . $v['uid'])->field("user_id,user_name,user_avat")->find();
            if (!$list[$k]['u']) {
              /*   $list[$k]['u']['user_avat'] = request()->domain().'/uploads/headimg/head.png';
                $list[$k]['u']['user_name'] = '游客'; */
...
//				$Pushs->getTypes($msg,$datas);
            ...
            //$ch=S($data['uid'].$data['touid']);
            //if(!$ch){
            //$fuser=M("user")->where("id=".$data['touid'])->find();
            //$this->duanxin($fuser['tel'],"您有一个新的客户提问，请及时查看！");
            //S($data['uid'].$data['touid'],1,3600);
            //}
