...
class Integral extends Common{
    ...
    public function exchangeList(){
        if(Request()->isPost()){
            $this->checkEmploy(Request()->post(),['user_id','token','page','low','high']);
            //判断用户的输入是否非法
            if(!$this->getUid(Request()->post("token"),intval(Request()->post("user_id")))){
            ...
            $sort="goods_id desc";
            //获得用户的基本信息
            $userInfo=$this->userObject->userInfo(['user_id'=>Request()->post("user_id")],"user_id,user_points");
            ...
            $goodsList=$this->model
                ...
                ->select();
            if($goodsList){
                $count=$this->model->where($where)->where('exchange_integral',$condition,$where_condition,'and')->count('goods_id');
    ...
    //积分商品详情
    public function intergalDetail(){
        if(Request()->post()){
            //积分商品id值
    ...
    function rightNow(){
        if(Request()->isPost()){
            $data=Request()->post();
            $this->checkEmploy($data,['token','user_id','goods_id','number']);
            $goodsId=Request()->post("goods_id/d");
            //判断用户是否存在
            if(!$this->getUid(Request()->post("token"),Request()->post("user_id/d"))){
                return json(['status'=>-1,'msg'=>'非法操作','data'=>[]]);
            }
            $number=Request()->post("number/d");
            //获得用户的积分信息
            $userInfo=$this->userObject->userInfo(['user_id'=>Request()->post("user_id/d")],"user_id,user_points");
             //查看该商品的积分
    ...
    function addOrder(){
        if(Request()->post()){
            $data=Request()->post();
            $this->checkEmploy($data,['token','user_id','goods_id','number','address_id','is_invoice']);
            //判断用户是否存在
            ...
            //查询收货地址信息
            $addressId=Request()->post("address_id/d");
...
//            $pro_name=$this->getRegion(['region_id'=>$address['addr_province']]);
//            $city_name=$this->getRegion(['region_id'=>$address['addr_city']]);
//            $area=$this->getRegion(['region_id'=>$address['addr_area']]);
            //进行添加订单
...
//                'area'=>$area,
   ...
   function integraluser(){
       if(Request()->isPost()){
           $this->checkEmploy(Request()->post(),['user_id','token']);
    ...
    */
	 public function getGift($user_id,$type,$res){
			...
			//元宝添加
				...
				// $end_time = time()+ $res['valid']*24*3600;
