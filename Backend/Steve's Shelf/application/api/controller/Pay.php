...
class Pay extends Common
{
    ...
    public function submit()
    {
        ...
        $params = [
            ...
            'title'        => $title,
            'amount'       => $type == 'alipay' ? $amount : $amount * 100,
            'method'       => $method,
            'openid'       => $openid,
            'auth_code'    => $auth_code,
            'notifyurl'    => $notifyurl,
		...
		return $this->payWeChatAliService->submitOrder($params);
    }
    ...
    public function wx_notifyx()
    {
        ...
        $notifyx = $this->payWeChatAliService->notifyx('wechat');
		// 回调处理
     ...
     * 阿里支付回调
     */
    public function ali_notifyx()
    {
        $xmlArray = input();
        $xmlJson = json_encode($xmlArray, true);
        $txt_path = 'uploads/ali_pay_log.txt';
        file_put_contents($txt_path,$xmlJson.PHP_EOL,FILE_APPEND);
        $notifyx = $this->payWeChatAliService->notifyx('alipay');
        // 回调处理
